...
idCompteAmi = document.getElementById("amis").value;
	if(idCompteAmi == ""){
		document.getElementById("repShareOption").innerHTML = "";
		document.getElementById("repShareContent").innerHTML = "";
		Session.setAttribute("folder","",function(){});
		Session.setAttribute("compteAmi","",function(){});
		
	}else{
		Session.setAttribute("compteAmi",idCompteAmi,function(){});
		Folder.getShareFolder(idCompteAmi,function(data){
			courantFolder = data.id;
...
function goDiaporama(){
	location.href = "diaporama.do?idFolder="+courantFolder+"&from=showPartageAmis.do";
}

function fillFolder(rep){
	courantFolder = rep;
  Folder.getFolders(rep,idCompteAmi,2,"folder", function(data) {
    var list = data;
    var i = 0;
    var html = "";
...
function goShowFile(id){
	window.location.href = "showFile.do?idFile="+id+"&from=showPartageAmis.do";
};
...
function goRepParent(){
	Folder.getParent(courantFolder,"folder",function(data){
		fillFolder(data.id)
	});
};
